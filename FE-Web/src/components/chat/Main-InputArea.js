...
const InputArea = (chatRoomId) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (files.length > 0) {
        ...
        try {
          const res = await axios.post(
            process.env.REACT_APP_API_URL + "/api/send-media",
            formData,
            {
              headers: {
            ...
            res.data.data.forEach((mediaData) => {
              ...
              try {
                if (socket?.connected) {
                  ...
                  alert("Không thể gửi tin nhắn. Vui lòng làm mới trang!");
                }
              } catch (socketError) {
                console.error("Socket emit error:", socketError);
                // Retry emit after 1 second if socket fails
                setTimeout(() => {
                  if (socket?.connected) {
                    socket.emit("message", data, mediaData._id);
                  }
      ...
      // Đảm bảo luôn reset trạng thái isSubmitting
  ...
  return (
    <div className="">
      {replyMessage.messageContent !== "" && showReplyMessage && (
        <Row
      ...
      <Stack direction="horizontal" gap={3} className="pe-2 border">
        <Form
          ...
          className="w-100 d-flex border-0 justify-content-evenly align-items-center position-relative"
              ...
              <Col key={index}>
                ...
                >
                  {files[index].type === "video/mp4" && (
                    <Badge
                      bg="infomation"
                      className="position-absolute top-0 left-0 m-1"
                    >
                      Video
                    </Badge>
                  )}
                  ...
                  />
                  <Button
                    style={{ backgroundColor: "transparent", padding: "0 5px" }}
                    size="sm"
                    className="position-absolute top-0 left-0 m-1"
